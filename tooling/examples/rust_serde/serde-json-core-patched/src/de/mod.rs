...
impl<'a> Deserializer<'a> {
    ...
    pub fn new(
        slice: &'a [u8],
        _string_unescape_buffer: Option<&mut [u8]>,
    ) -> Deserializer<'a> {
        Deserializer {
            slice,
            index: 0,
            ...
            match self.peek() {
                Some(b'"') => {
                    ...
                    let leading_backslashes = |index: usize| -> usize {
                        let mut count = 0;
                        loop {
                            // SAFETY: index > start (we found a '"' after start),
                            // and the loop terminates at the opening '"' at start-1
                            if unsafe { *self.slice.get_unchecked(index - count - 1) } == b'\\' {
                                count += 1;
                            } else {
                                return count;
                            }
                    ...
                    if is_escaped {
                        ...
                        // SAFETY: start <= end <= self.slice.len() (bounds validated by peek loop),
                        // and JSON input is ASCII from our own serializer
                        return Ok(unsafe { str::from_utf8_unchecked(self.slice.get_unchecked(start..end)) });
                    }
                }
...
impl<'a, 'de> de::Deserializer<'de> for &'a mut Deserializer<'de> {
        ...
        V: Visitor<'de>,
    {
        ...
        // We never use unescape buffers - always return borrowed str directly.
        // This eliminates the EscapedStringFragmentIter, char::encode_utf8,
        // and copy_from_slice code paths.
    ...
    where
        V: Visitor<'de>,
    {
        // EscapedStr support removed for size optimization
