...
static __inline void wideMultiply(rep_t a, rep_t b, rep_t *hi, rep_t *lo) {
  // Use 16-bit partial products to avoid 64-bit multiply libcalls.
  const uint32_t a_lo = a & 0xFFFFu;
  const uint32_t a_hi = a >> 16;
  const uint32_t b_lo = b & 0xFFFFu;
  const uint32_t b_hi = b >> 16;

  const uint32_t p0 = a_lo * b_lo;
  const uint32_t p1 = a_lo * b_hi;
  const uint32_t p2 = a_hi * b_lo;
  const uint32_t p3 = a_hi * b_hi;
...
static __inline void wideRightShiftWithSticky(rep_t *hi, rep_t *lo,
  ...
  if (count < typeWidth) {
    ...
    *hi = *hi >> count;
  } else if (count == typeWidth) {
    const bool sticky = *lo != 0;
    *lo = *hi | sticky;
    *hi = 0;
  } else if (count < 2 * typeWidth) {
    const unsigned int shift = count - typeWidth;
    const bool sticky = (*hi << (typeWidth - shift)) | *lo;
    *lo = *hi >> shift | sticky;
    *hi = 0;
  } else {
